...
abstract class Bombalcohol {
	...
	private static String[] resultKind = { "소주", "맥주", "콜라", "사이다" };
	private static double[] result = new double[resultKind.length];
	private static double[] resultJan = new double[resultKind.length];
	private String kind;
	private int jan;
	...
	public void setResult(double[] result) {
		for (int i = 0; i < result.length; i++) {
			if (result[i] != 0)
				Bombalcohol.result[i] += result[i];
		}
	}

	public static double[] getResultJan() {
		return resultJan;
	}

	public static void setResultJan(double[] resultJan) {
		for (int i = 0; i < result.length; i++) {
			if (resultJan[i] != 0)
				Bombalcohol.resultJan[i] += resultJan[i];
		}
	}

	abstract void calc();

	abstract void push(int jan);
	...
	void resultShow() {
		...
		for (int i = 0; i < Bombalcohol.result.length; i++) {
			result += resultKind[i] + "\t" + resultJan[i] + "\t" + this.result[i] + "\n";
		}
...
class Soju extends Bombalcohol {
	...
	void push(int jan) {
		...
		result[0] += this.soju;
		setResult(result);
		double[] resultJan = getResultJan();
		resultJan[0] += jan;
		setResultJan(resultJan);
