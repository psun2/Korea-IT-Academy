...
$(document).ready(function () {
  loadPage(page, pageRows); // 페이지 최조 로딩되면, 게시판 1page 분량 가져오기
...
function updateList(jsonObj) {
  ...
  if (jsonObj.status === 'OK') {
    ...
    for (i = 0; i < count; i++) {
      ...
      result +=
        "<td><span class='subject' data-uid='" +
        items[i].uid +
        "'>" +
        items[i].subject +
        '</span></td>\n';
    ...
    // 페이지 정보 업데이트
    $('#pageinfo').text(
      `${jsonObj.page}/${jsonObj.totalpage}페이지, ${jsonObj.totalcnt}개의 글`,
    );

    // pageRows
    var txt = `<select id='rows' onchange='changPageRows();'>`;
    txt += `<option value=${10} ${
