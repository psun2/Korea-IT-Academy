...
public class KeyEventMain extends JFrame implements KeyListener{
	...
	class Enemy extends JLabel{
		...
		public Enemy() {
			...
			KeyEventMain.this.add(this);
		}
		
		void move() {
			x = getX()+dirX; y= getY()+dirY;
			
			if(x<0 || x>500-10-size) dirX*=-1;
			if(y<0|| y>600-35-size) dirY*=-1;
			
			setLocation(x, y);
			
			if(x<me.getX()+50 && x+size>me.getX() &&
					y<me.getY()+50 && y+size>me.getY() ) {
				chk = true;
