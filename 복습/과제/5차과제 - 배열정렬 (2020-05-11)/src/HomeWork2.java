...
//		12 => 2,3,4,6 : 4개 
		...
		// 2. 약분의 크기를 비교하기 위해 abb의 배열을 복사한 abbSort 배열을 생성합니다.
		int[] abbSort = new int[arr.length];
		for (int i = 0; i < abb.length; i++) {
			abbSort[i] = abb[i];
		}

		// 3. 복사한 abbSort배열을 정렬합니다.
		...
		// 정렬되기전 약분의 갯수를 담은 abb 배열과 정렬후의 abbSort 배열을 비교하여,
		// 해당값의 index를 얻어 옵니다.
...
//		for (int i = 0; i < abbSortIndex.length; i++) {
//
//			int a = 0;
//			
//			for (int j = 0; j < abbSortIndex.length; j++) {
//				if(abbSortIndex[i] < abbSortIndex[j]) {
//					a = abbSortIndex[i];	
//					abbSortIndex[i] = abbSortIndex[j];
//					abbSortIndex[j] = a;
//				break;
//				}
//			}
